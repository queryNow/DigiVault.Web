...
import { ComplianceMonitorAgent, DocumentAnalystAgent, InvestmentAdvisorAgent, RiskAnalystAgent } from '../../utils/services/ai/agents';
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${data.analysis.risk.level === 'high' ? 'bg-red-100 text-red-800' :
            ...
            <div className="space-y-4">
              {data.recommendations.map((rec: any, index: number) => (
                <div key={index} className="flex items-start p-4 bg-gray-50 rounded-lg">
                  <div className={`flex-shrink-0 w-2 h-2 mt-2 rounded-full ${rec.priority === 'high' ? 'bg-red-500' :
              ...
              <div>
                ...
                <div className="space-y-2">
                  {assets.map(asset => (
                    <div
                      key={asset.id}
                      className={`flex items-center justify-between p-2 rounded cursor-pointer ${selectedContext.some(item => item.id === asset.id)
              ...
              <div>
                ...
                <div className="space-y-2">
                  {documents.map(doc => (
                    <div
                      key={doc.id}
                      className={`flex items-center justify-between p-2 rounded cursor-pointer ${selectedContext.some(item => item.id === doc.id)
                        ? 'bg-indigo-50 border border-indigo-200'
                        : 'hover:bg-gray-50'
                        }`}
                      onClick={() => toggleContextItem({
